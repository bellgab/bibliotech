                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $category->name }}</td>
                                        <td class="text-end">{{ $category->books_count }}</td>
                                        <td class="text-end">{{ $category->total_copies }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="card-body">
                    ...
                    <div class="mt-4">
                        <div class="row text-center">
                            @foreach($userActivityStats as $activity)
                            <div class="col-6 col-md-3 mb-3">
                                <div class="card bg-light">
                                    <div class="card-body p-2">
                                        <h6 class="card-title small mb-1">{{ $activity->activity_level }}</h6>
                                        <p class="card-text h5 mb-0 text-primary">{{ $activity->user_count }}</p>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <span class="badge bg-success">{{ $month['returns'] }}</span>
                                    </td>
                                    <td class="text-end">
                                        @if($month['borrows'] > 0)
                                            @php
                                            ...
                                            @endphp
                                            <span class="badge bg-{{ $returnRate >= 80 ? 'success' : ($returnRate >= 60 ? 'warning' : 'danger') }}">
                                                {{ $returnRate }}%
                                            </span>
                                        @else
                                    ...
                                    <td class="text-end">
                                        ...
                                        @endphp
                                        <span class="badge bg-{{ $outstanding > 0 ? 'warning' : 'secondary' }}">
                                            {{ $outstanding }}
                                        </span>
                                    </td>
                                </tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const monthlyTrendsChart = new Chart(monthlyTrendsCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: {!! json_encode(array_column($monthlyStats, 'borrows')) !!},
                borderColor: 'rgb(75, 192, 192)',
                backgroundColor: 'rgba(75, 192, 192, 0.2)',
                tension: 0.1
            }, {
                label: 'Books Returned',
                data: {!! json_encode(array_column($monthlyStats, 'returns')) !!},
    ...
    const userActivityChart = new Chart(userActivityCtx, {
        type: 'bar',
        data: {
            labels: {!! json_encode($userActivityStats->pluck('activity_level')) !!},
            datasets: [{
                label: 'Number of Users',
                data: {!! json_encode($userActivityStats->pluck('user_count')) !!},
                backgroundColor: [
                    'rgba(255, 99, 132, 0.8)',
                    'rgba(54, 162, 235, 0.8)',
                    'rgba(255, 205, 86, 0.8)',
